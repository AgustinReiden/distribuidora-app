  ...
  const VistaDashboard = () => {
    ...
    const aplicarFechasPersonalizadas = () => {
      if (fechaDesdeLocal || fechaHastaLocal) {
        cambiarPeriodo('personalizado', fechaDesdeLocal || null, fechaHastaLocal || null);
      }
    };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow p-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {['hoy', 'semana', 'mes', 'anio', 'historico', 'personalizado'].map(periodo => (
              <button
                key={periodo}
          ...
          {mostrarFechasPersonalizadas && (
            <div className="mt-4 flex flex-wrap items-end gap-3">
              <div>
                ...
                <input type="date" value={fechaDesdeLocal} onChange={e => setFechaDesdeLocal(e.target.value)} className="px-3 py-2 border rounded-lg text-sm" />
              </div>
              <div>
                <label className="block text-xs text-gray-500 mb-1">Hasta</label>
                <input type="date" value={fechaHastaLocal} onChange={e => setFechaHastaLocal(e.target.value)} className="px-3 py-2 border rounded-lg text-sm" />
              </div>
              <button onClick={aplicarFechasPersonalizadas} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">Aplicar</button>
            </div>
          )}
        </div>
        ...
        {loadingMetricas ? <LoadingSpinner /> : (
          <>
            {/* Métricas principales del período seleccionado */}
            ...
            {/* Estados de pedidos (siempre total) */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-orange-50 border border-orange-200 rounded-lg p-4"><div className="flex items-center space-x-2"><Package className="w-5 h-5 text-orange-600" /><span className="text-orange-800 font-medium">En preparacion</span></div><p className="text-3xl font-bold text-orange-600 mt-2">{metricas.pedidosPorEstado.en_preparacion || 0}</p></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-xl shadow p-4">
                <h3 className="font-semibold mb-4">Top 5 Productos ({periodoLabels[filtroPeriodo]})</h3>
