...
// `obtener_resumen_cuenta_cliente` (definida en migrations/000_baseline.sql).
//
...
export const fichaClienteTool: Tool<FichaClienteParams, FichaClienteResult> = {
  ...
  handler: async ({ cliente_id }, ctx) => {
    ...
    // La RPC puede retornar {error: "..."} si auth.uid() es null (poco probable
    // bajo service_role pero defendámonos).
